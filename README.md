...
- **ค้นหาคำขอจัดซื้อ** — ค้นหาได้ทั้งชื่อวัสดุและรหัสวัสดุ (cat_code)
- **แสดงรหัสวัสดุ** (cat_code) เป็นคอลัมน์แยกในตาราง
...
- สร้าง notification อัตโนมัติ 5 ประเภท: หมดสต็อก (🔴), สต็อกใกล้หมด (🟡), หมดอายุแล้ว (🔴), ใกล้หมดอายุ (🟠), สถานะจัดซื้อเปลี่ยน (🔵)
- **แจ้งเตือนสถานะจัดซื้อ** — สร้าง notification ทุกครั้งที่สถานะคำขอจัดซื้อเปลี่ยน (แจ้งคำขอ → กำลังสั่งซื้อ → กำลังจัดส่ง → ส่งถึงแล้ว → รับแล้ว)
- ไม่สร้างซ้ำถ้ามี notification ที่ยังไม่อ่านของ item+type เดียวกันอยู่แล้ว (ยกเว้น procurement_status ที่สร้างทุกครั้ง)
- กดที่ notification → mark เป็นอ่านแล้ว + นำทางไปหน้าที่เกี่ยวข้อง (สถานะจัดซื้อ → หน้าจัดซื้อ, อื่นๆ → หน้าวัสดุ)
- Badge ตัวเลข unread count บน icon กระดิ่ง
- Auto-refresh ทุก 60 วินาที
- ลบ notification ที่อ่านแล้วอัตโนมัติทุกรอบ generate

### อื่นๆ
- สรุปรายงานรายเดือน/รายปี (SVG charts)
- **ปริ้นรายการวัสดุ** — เลือกคอลัมน์ที่ต้องการแสดงได้ (ชื่อ, รหัสวัสดุ, หมวดหมู่, ขนาด, เกรด, ยี่ห้อ, หน่วย, คงเหลือ, วันหมดอายุ)
- เข้าใช้งานผ่าน Internet ด้วย Cloudflare Tunnel
- **Procurement default page** — ผู้ใช้ role procurement login แล้วเข้าหน้าจัดซื้อโดยตรง
...
**Query params สำหรับ GET /api/procurement:**

| Param | คำอธิบาย | ค่าที่ใช้ได้ |
|-------|----------|-------------|
| `status` | กรองตามสถานะ | `requested`, `ordering`, `shipping`, `delivered`, `received` |
| `requested_by` | กรองตามผู้แจ้งคำขอ | ID ของ user |
| `search` | ค้นหาชื่อวัสดุหรือรหัสวัสดุ | ข้อความ |
| `page` | หน้า | ตัวเลข (default: 1) |
| `limit` | จำนวนต่อหน้า | ตัวเลข (default: 20) |
...
- `received_by` — ระบบบันทึก user ที่ login อยู่ (กดยืนยันรับ = ผู้รับพัสดุ) อัตโนมัติ
...
| `received_by` | INTEGER FK | อ้างอิง users.id (ผู้รับพัสดุ / ผู้กดยืนยันรับ) |
...
|--------|------|----------|
| `id` | INTEGER PK | Auto increment |
| `type` | TEXT NOT NULL | `out_of_stock` / `low_stock` / `expiring` / `expired` / `procurement_status` |
